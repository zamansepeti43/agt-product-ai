...
export default function ProductStudio() {
  ...
  async function exportZip() {
    ...
    try {
      ...
      const blob = await r.blob(); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "AGT-Product-AI-export.zip"; a.click(); URL.revokeObjectURL(url);
  ...
  return <main style={styles.page}>
    ...
    <section style={styles.card}><div style={styles.cardHead}><div><p style={styles.kicker}>2 • AI MOTORU</p><h2 style={styles.h2}>İstediğin yapay zekâyı bağla</h2><p style={styles.muted}>Anahtarlar tarayıcıda saklanmaz; istek sırasında sunucuya iletilir.</p></div><button style={styles.secondary} onClick={openFinder}>🔎 API Finder</button></div><div style={styles.providerGrid}>{providers.map((p) => <button key={p.id} onClick={() => selectProvider(p.id)} style={{ ...styles.provider, ...(provider === p.id ? styles.active : {}) }}><strong>{p.icon} {p.label}</strong><small>{p.note}</small></button>)}</div>
      {provider !== "auto-free" && provider !== "comfyui" && <div style={styles.fields}>
        {(provider === "cloudflare") && <input value={accountId} onChange={(e) => setAccountId(e.target.value)} placeholder="Cloudflare Account ID" />}
        <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder={provider === "cloudflare" ? "Cloudflare API Token" : `${selectedProvider.label} API Key`} autoComplete="off" />
        {(provider === "openai" || provider === "custom-openai" || provider === "cloudflare" || provider === "aihorde" || provider === "gemini") && <input value={model} onChange={(e) => setModel(e.target.value)} placeholder="Model adı" />}
        {(provider === "openai" || provider === "custom-openai") && <input value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder="API Base URL" />}
      </div>}
      {provider === "auto-free" && <p style={styles.help}>🆓 Önce yapılandırılmış ComfyUI, sonra bağlı AI Horde denenir. Gemini/OpenAI/Cloudflare otomatik ücretsiz havuza dahil değildir.</p>}
...
const styles: Record<string, React.CSSProperties> = {
  page:{minHeight:"100vh",background:"#f7f7fa",color:"#171923",fontFamily:"Inter,ui-sans-serif,system-ui,sans-serif",padding:"24px",maxWidth:1200,margin:"0 auto"},
  header:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0 28px"},
  ...
  hero:{display:"grid",gridTemplateColumns:"1.25fr .75fr",gap:24,alignItems:"stretch",marginBottom:24}, kicker:{fontSize:11,fontWeight:800,letterSpacing:1.4,color:"#777b8d",margin:"0 0 8px"},h1:{fontSize:"clamp(38px,6vw,68px)",lineHeight:.98,letterSpacing:-3,margin:"0 0 20px"},h2:{fontSize:24,margin:"0 0 5px",letterSpacing:-.7},lead:{fontSize:18,lineHeight:1.6,maxWidth:700,color:"#616575"},drop:{border:"2px dashed #c8cad5",borderRadius:24,background:"#fff",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:280,padding:24,textAlign:"center",gap:10},card:{background:"#fff",border:"1px solid #e4e5eb",borderRadius:22,padding:24,marginBottom:18,boxShadow:"0 8px 30px rgba(20,20,40,.04)"},cardHead:{display:"flex",justifyContent:"space-between",gap:16,alignItems:"center",marginBottom:18},muted:{color:"#747889",fontSize:13},grid:{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10},mode:{border:"1px solid #e2e3ea",background:"#fafafd",borderRadius:15,padding:16,textAlign:"left",display:"flex",flexDirection:"column",gap:7},active:{border:"2px solid #171923",background:"#f1f1f6"},providerGrid:{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10},provider:{border:"1px solid #e2e3ea",background:"#fafafd",borderRadius:14,padding:14,textAlign:"left",display:"flex",flexDirection:"column",gap:5},fields:{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginTop:14},controls:{display:"flex",gap:14,alignItems:"flex-start"},generate:{width:"100%",border:0,borderRadius:14,padding:"16px 20px",background:"#171923",color:"#fff",fontWeight:800,fontSize:16,marginTop:16},secondary:{border:"1px solid #dfe0e8",background:"#fff",borderRadius:12,padding:"10px 14px",fontWeight:700},help:{fontSize:13,color:"#696d7d",background:"#f5f5f8",padding:12,borderRadius:12},error:{marginTop:14,padding:12,borderRadius:12,background:"#fff0f0",color:"#a32727"},success:{marginTop:14,padding:12,borderRadius:12,background:"#effaf1",color:"#247332"},results:{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12},result:{width:"100%",aspectRatio:"1/1",objectFit:"cover",borderRadius:14,background:"#f0f0f3"},overlay:{position:"fixed",inset:0,background:"rgba(10,10,20,.5)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,zIndex:20},modal:{width:"min(900px,100%)",maxHeight:"90vh",overflow:"auto",background:"#fff",borderRadius:22,padding:24}
};
