...
function assertProviderAssetUrl(rawUrl: string) {
  ...
  if (url.origin !== allowedOrigin || url.pathname !== "/view") {
...
function makeZip(files: Array<{ name: string; data: Uint8Array }>) {
  ...
  for (const file of files) {
    ...
    writeU32(view, 18, file.data.byteLength);
    writeU32(view, 22, file.data.byteLength);
    ...
    writeU32(recordView, 0, 0x02014b50);
    writeU16(recordView, 4, 20);
    writeU16(recordView, 6, 20);
    writeU16(recordView, 8, 0x0800);
    writeU16(recordView, 10, 0);
    writeU16(recordView, 12, 0);
    writeU16(recordView, 14, 0);
    writeU32(recordView, 16, crc);
    writeU32(recordView, 20, file.data.byteLength);
    writeU32(recordView, 24, file.data.byteLength);
    writeU16(recordView, 28, name.length);
    writeU16(recordView, 30, 0);
    writeU16(recordView, 32, 0);
    writeU16(recordView, 34, 0);
    writeU16(recordView, 36, 0);
    writeU32(recordView, 38, 0);
    writeU32(recordView, 42, offset);
    record.set(name, 46);
    central.push(record);
...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as { assets?: ZipAsset[] };
    const assets = Array.isArray(body.assets) ? body.assets.slice(0, MAX_ASSETS) : [];
    if (!assets.length) {
      return NextResponse.json({ error: "ZIP için en az bir görsel gerekli." }, { status: 400 });
    }
    ...
    for (let index = 0; index < assets.length; index += 1) {
      ...
      if (!asset || typeof asset.url !== "string") throw new Error("Geçersiz görsel kaydı.");
      const url = assertProviderAssetUrl(asset.url);
      const response = await fetch(url, { cache: "no-store" });
      if (!response.ok) throw new Error(`ComfyUI çıktısı alınamadı (${response.status}).`);

      const contentLength = Number(response.headers.get("content-length") || 0);
      if (contentLength && totalBytes + contentLength > MAX_TOTAL_BYTES) {
        throw new Error("ZIP toplam boyutu 50 MB sınırını aşıyor.");
      }
    ...
    return new NextResponse(zip.buffer as ArrayBuffer, {
      ...
      headers: {
        ...
        "Content-Disposition": 'attachment; filename="AGT-Product-AI-export.zip"',
        "Cache-Control": "no-store",
      },
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "ZIP oluşturulamadı.";
