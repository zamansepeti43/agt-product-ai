...
export default function Home() {
  ...
  async function generateCatalog() {
    ...
    try {
      const response = await fetch("/api/catalog", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: catalogName, category: catalogCategory, brand: catalogBrand, keywords: catalogKeywords.split(",").map((x) => x.trim()).filter(Boolean), marketplace: "generic", language: "tr" }) });
  ...
  return (
    <main className="shell">
      <header className="topbar"><div className="brand"><span className="brand-mark">AGT</span><span>Product AI</span></div><span className="status">v0.5 • catalog studio</span></header>
      ...
      <section className="workspace"><div className="section-head"><div><p className="eyebrow">ÜRETİM</p><h2>İçerik türünü seç</h2></div><span className="device-note">📱 PWA • 💻 Windows</span></div>
        <div className="modes">{GENERATION_PRESETS.map((preset) => <button key={preset.id} className={`mode ${mode === preset.id ? "selected" : ""}`} onClick={() => setMode(preset.id)}><span className="mode-icon">{preset.id === "hero" ? "🏪" : preset.id === "white" ? "⚪" : preset.id === "studio" ? "✨" : preset.id === "lifestyle" ? "🏠" : preset.id === "detail" ? "🔍" : "📱"}</span><span><strong>{preset.label}</strong><small>{preset.description}</small></span></button>)}</div>
        ...
        <div className="generation-controls"><label><span>Ürün başına görsel</span><select value={count} onChange={(e) => setCount(Number(e.target.value))}>{[1,2,3,4].map((v) => <option key={v} value={v}>{v} görsel</option>)}</select></label><label className="prompt-field"><span>Ek talimat <small>isteğe bağlı</small></span><textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} maxLength={1200} placeholder="Örn. ürünü değiştirme, premium doğal ışık, sade arka plan…" rows={3} /></label></div>
        {error && <div className="notice error">{error}</div>}{job && <div className="notice success"><strong>✓ {job.message}</strong><small>Job: {job.id}</small></div>}
        {assets.length > 0 && <div className="results"><div className="results-head"><div><span className="eyebrow">SONUÇ</span><h2>{assets.length} görsel hazır</h2></div><button className="export-button" onClick={exportZip} disabled={exporting || busy}>{exporting ? "ZIP hazırlanıyor…" : "⬇ Tümünü ZIP indir"}</button></div><div className="asset-grid">{assets.map((asset) => { const previewUrl = assetPreviewUrl(asset.url); return <a className="asset-card" key={asset.id} href={previewUrl} target="_blank" rel="noreferrer"><img src={previewUrl} alt="Üretilen ürün görseli" loading="lazy" /><span>{asset.mode} • {asset.width}×{asset.height}</span></a>; })}</div></div>}
        <div className="catalog-card"><div className="results-head"><div><span className="eyebrow">KATALOG / SEO</span><h2>Ürün metnini hazırla</h2></div></div><div className="catalog-grid"><input value={catalogName} onChange={(e) => setCatalogName(e.target.value)} placeholder="Ürün adı" /><input value={catalogCategory} onChange={(e) => setCatalogCategory(e.target.value)} placeholder="Kategori" /><input value={catalogBrand} onChange={(e) => setCatalogBrand(e.target.value)} placeholder="Marka (opsiyonel)" /><input value={catalogKeywords} onChange={(e) => setCatalogKeywords(e.target.value)} placeholder="Anahtar kelimeler, virgülle ayır" /></div><button className="export-button catalog-action" onClick={generateCatalog} disabled={catalogBusy}>{catalogBusy ? "Hazırlanıyor…" : "Katalog metnini oluştur"}</button>{catalog && <div className="catalog-result"><strong>{catalog.title}</strong><p>{catalog.shortDescription}</p><p>{catalog.description}</p><div><b>Etiketler:</b> {catalog.tags.join(" • ")}</div><div><b>SEO:</b> {catalog.seoKeywords.join(", ")}</div></div>}</div>
        <div className="action-row"><div><strong>{files.length ? `${files.length} fotoğraf hazır` : "Önce ürün fotoğrafını seç"}</strong><span className="muted"> • {selected.label} • ürün başına {count} çıktı</span></div><button className="generate" disabled={!files.length || busy || exporting} onClick={startGeneration}>{busy ? "İşlem çalışıyor…" : files.length > 1 ? "Toplu Üretimi Başlat →" : "Üretmeye Başla →"}</button></div>
      </section>
      <section className="pipeline"><div><span>01</span><strong>Fotoğraf</strong><small>Tekli / katalog</small></div><i>→</i><div><span>02</span><strong>Hazırla</strong><small>Ürün + preset</small></div><i>→</i><div><span>03</span><strong>AI İşleme</strong><small>Qwen / ComfyUI</small></div><i>→</i><div><span>04</span><strong>Katalog</strong><small>SEO / ZIP</small></div></section>
    </main>
  );
}
