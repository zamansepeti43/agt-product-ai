...
export default function Home() {
  ...
  async function startGeneration() {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : "Beklenmeyen bir hata oluştu."); }
  ...
  async function exportZip() {
    ...
    try {
      ...
      link.href = url; link.download = "AGT-Product-AI-export.zip"; document.body.appendChild(link); link.click(); link.remove();
      setTimeout(() => URL.revokeObjectURL(url), 1000);
    } catch (err) { setError(err instanceof Error ? err.message : "ZIP indirilemedi."); }
    finally { setExporting(false); }
  }

  return (
    <main className="shell">
      <header className="topbar"><div className="brand"><span className="brand-mark">AGT</span><span>Product AI</span></div><span className="status">v0.4 • batch studio</span></header>
      <section className="hero">
        <div><p className="eyebrow">AGT STUDIO</p><h1>Ürün fotoğrafını<br /><span>satış içeriğine</span> dönüştür.</h1><p className="lead">Tek fotoğraf veya ürün kataloğunu yükle. Ürünü koruyarak e-ticaret, stüdyo, lifestyle, detay ve sosyal medya içerikleri üret.</p><div className="trust-row"><span>✓ JPG / PNG / WEBP</span><span>✓ 12 MB / görsel</span><span>✓ 6 ürün / batch</span><span>✓ Telefon kamerası</span></div></div>
        <label className={`dropzone ${preview ? "has-preview" : ""}`}><input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" multiple onChange={(e) => selectFiles(Array.from(e.target.files ?? []))} />{preview ? <img src={preview} alt="Ürün önizleme" /> : <div className="upload-icon">＋</div>}<strong>{files.length ? `${files.length} ürün seçildi` : "Ürün fotoğraflarını yükle"}</strong><small>{files.length ? `${files[0].name}${files.length > 1 ? ` + ${files.length - 1} ürün` : ""}` : "Tek ürün veya aynı anda 6 ürüne kadar katalog seçebilirsin"}</small>{files.length > 0 && <span className="change-file">Fotoğrafları değiştir</span>}</label>
      </section>
      <section className="workspace">
        <div className="section-head"><div><p className="eyebrow">ÜRETİM</p><h2>İçerik türünü seç</h2></div><span className="device-note">📱 PWA • 💻 Windows</span></div>
