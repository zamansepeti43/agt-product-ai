...
function renderLibrary(kind){
  ...
  items.forEach(item=>{
    ...
    const img=document.createElement("img");img.loading="lazy";img.decoding="async";img.src=item.url;img.draggable=false;img.alt="Üretilen ürün görseli";
...
function enableSheetSwipeClose(){
  const modal=$("#modal"),sheet=modal?.querySelector(".sheet"),body=$("#sheetBody");
  if(!modal||!sheet||sheet.dataset.swipeBound)return;
  ...
  sheet.addEventListener("touchstart",e=>{
    ...
    tracking=!body || body.scrollTop<=1;
  },{passive:true});
  sheet.addEventListener("touchend",e=>{
    ...
    if(dy>72&&Math.abs(dy)>Math.abs(dx)*1.25){
      modal.classList.add("hidden");state.nav="home";renderNav();
    }
  },{passive:true});
}
...
function openSettings(){const t=L();$("#modal").classList.remove("hidden");enableSheetSwipeClose();$("#sheetTitle").textContent=t.settingsNav;$("#sheetBody").innerHTML='<div class="setting-row"><div><b>'+t.language+'</b><small>Türkçe / English</small></div><button class="choice" id="languageChoice">'+(state.lang==="tr"?"🇹🇷 Türkçe":"🇬🇧 English")+'</button></div><div class="setting-row"><div><b>'+t.notifications+'</b><small>'+t.notifyOn+'</small></div><button class="toggle '+(state.notify?"on":"")+'" id="notifyToggle"><span></span></button></div><div class="about">'+t.app+'<br><small>v1.0</small></div>';$("#close").onclick=()=>$("#modal").classList.add("hidden");$("#languageChoice").onclick=()=>{state.lang=state.lang==="tr"?"en":"tr";write("agt-lang",state.lang);$("#modal").classList.add("hidden");render()};$("#notifyToggle").onclick=()=>{state.notify=!state.notify;write("agt-notify",state.notify);openSettings()}}
function openProviderModal(){$("#modal").classList.remove("hidden");enableSheetSwipeClose();$("#sheetTitle").textContent=L().addTitle;$("#sheetBody").innerHTML='<div class="provider-grid" id="providerGrid"></div><div id="form" class="provider-form-host" hidden></div>';$("#close").onclick=()=>$("#modal").classList.add("hidden");const grid=$("#providerGrid"),form=$("#form");if(!grid||!form)return;grid.hidden=false;grid.classList.remove("is-hidden");grid.style.display="grid";form.hidden=true;form.style.display="none";grid.replaceChildren();PROVIDERS.forEach(p=>{const connected=state.slots.some(s=>s.provider===p[0]);const btn=document.createElement("button");btn.type="button";btn.className=connected?"connected-provider":"";btn.innerHTML="<strong>"+p[2]+"</strong><span>"+p[1]+"</span><small>"+(connected?"✓ Bağlı":(p[3]?L().free:"API key"))+"</small>";btn.onclick=()=>showForm(p[0]);grid.appendChild(btn)})}
function showForm(pid){
  const p=provider(pid),old=state.slots.find(x=>x.provider===pid),t=L();
  const grid=$("#providerGrid"),body=$("#sheetBody");
  if(!p||!body){toast("AI sağlayıcısı bulunamadı","error");return}
  let form=$("#form");
  if(!form){form=document.createElement("div");form.id="form";form.className="provider-form-host";body.appendChild(form)}
  let models=[...new Set([...(pid==="pollinations"?MODEL_CATALOG[pid]:cachedModels(pid)),...(MODEL_CATALOG[pid]||[]),old&&old.model,p[5]].filter(Boolean))];
  let h='<div class="provider-form-head"><button type="button" class="provider-back" id="providerBack">‹ Geri</button><div><small>AGT STUDIO</small><h3>'+p[2]+" "+esc(p[1])+'</h3></div></div>';
  h+='<div class="form"><div class="provider-selected"><span>'+p[2]+'</span><div><b>'+esc(p[1])+'</b><small>'+esc(p[3]?L().free:"API bağlantısı")+'</small></div></div>';
  h+='<label>'+t.name+'<input id="label" value="'+esc(old&&old.label||p[1])+'"></label>';
  if(p[4])h+='<label>'+t.key+'<input id="key" type="password" placeholder="'+(old&&old.key?"Kayıtlı anahtar — değiştirmek için yenisini gir":"API anahtarınızı girin")+'"></label>';
  if(old&&old.key)h+='<small class="key-saved-note">🔒 API anahtarı kayıtlı. Boş bırakırsan mevcut anahtar korunur.</small>';
  if(pid==="cloudflare")h+='<label>'+t.account+'<input id="account" value="'+esc(old&&old.accountId||"")+'"></label>';
  if(["custom","generic","comfyui"].includes(pid))h+='<label>'+t.base+'<input id="base" value="'+esc(old&&old.baseUrl||(pid==="comfyui"?"http://127.0.0.1:8188":""))+'"></label>';
  if(pid==="comfyui")h+='<label>Workflow<textarea id="workflow">'+esc(old&&old.workflow||"")+'</textarea></label>';
  h+='<label class="model-field">'+t.model+'<div class="model-picker"><select id="model" aria-label="'+t.model+'">'+renderModelOptions(pid,models)+'</select><span class="model-chevron">⌄</span><button type="button" id="refreshModels" aria-label="Modelleri yenile">↻</button></div><small id="modelStatus">'+(models.length?models.length+" model hazır":"Varsayılan model")+'</small></label>';
  h+='<button class="save-btn" id="save">'+t.save+'</button><small class="secure-note">'+t.secure+'</small></div>';
  form.innerHTML=h;
  if(grid){grid.hidden=true;grid.style.display="none"}
  form.hidden=false;form.removeAttribute("hidden");form.style.display="block";
  const back=$("#providerBack");if(back)back.onclick=()=>{form.hidden=true;form.style.display="none";if(grid){grid.hidden=false;grid.style.display="grid"}};
  const model=$("#model");if(old&&old.model&&models.includes(old.model)&&model)model.value=old.model;
  const refresh=$("#refreshModels");if(refresh)refresh.onclick=()=>refreshModelSelect(pid,{...(old||{}),key:$("#key")?.value.trim()||old?.key||"",baseUrl:$("#base")?.value.trim()||old?.baseUrl||"",accountId:$("#account")?.value.trim()||old?.accountId||""});
  const save=$("#save");if(save)save.onclick=()=>saveProvider(pid,old,p);
  if(["gemini","openai","pollinations","aihorde","cloudflare","puter"].includes(pid)&&!cachedModels(pid).length)refreshModelSelect(pid,{...(old||{}),key:old?.key||"",baseUrl:old?.baseUrl||"",accountId:old?.accountId||""});
}
...
render();
/* Standalone APK build: provider connection form fix included. */
