...
function render(){
  ...
  const resultsKey=state.results.map(x=>x.url).join("|");
...
function renderResults(){const box=$("#results");if(!box)return;box.replaceChildren();state.results.forEach((x,i)=>{const f=document.createElement("figure"),img=document.createElement("img"),b=document.createElement("button");img.src=x.url;b.textContent=L().download;b.onclick=()=>download(x,i);f.append(img,b);box.appendChild(f)})}
