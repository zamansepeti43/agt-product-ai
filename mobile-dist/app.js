...
function showForm(pid){const p=provider(pid),old=state.slots.find(x=>x.provider===pid),t=L();$("#providerGrid").style.display="none";let models=[...new Set([...(pid==="pollinations"?MODEL_CATALOG[pid]:cachedModels(pid)),...(MODEL_CATALOG[pid]||[]),...(pid==="pollinations"?"":old&&old.model)].filter(Boolean))];let h='<div class="form"><h3>'+p[2]+" "+p[1]+'</h3><label>'+t.name+'<input id="label" value="'+esc(old&&old.label||p[1])+'"></label>';if(p[4])h+='<label>'+t.key+'<input id="key" type="password" placeholder="••••••••"></label>';if(pid==="cloudflare")h+='<label>'+t.account+'<input id="account" value="'+esc(old&&old.accountId||"")+'"></label>';if(["custom","generic","comfyui"].includes(pid))h+='<label>'+t.base+'<input id="base" value="'+esc(old&&old.baseUrl||(pid==="comfyui"?"http://127.0.0.1:8188":""))+'"></label>';if(pid==="comfyui")h+='<label>Workflow<textarea id="workflow">'+esc(old&&old.workflow||"")+'</textarea></label>';h+='<label class="model-field">'+t.model+'<div class="model-picker"><select id="model" aria-label="'+t.model+'">'+models.map(x=>"<option value=\""+esc(x)+"\">"+esc(x)+"</option>").join("")+'</select><span class="model-chevron">⌄</span><button type="button" id="refreshModels" aria-label="Modelleri yenile">↻</button></div><small id="modelStatus">'+(models.length?models.length+" model hazır":"Model seç")+'</small></label><button class="save-btn" id="save">'+t.save+'</button><small class="secure-note">'+t.secure+'</small></div>';$("#form").innerHTML=h;if(old&&old.model&&models.includes(old.model))$("#model").value=old.model;$("#refreshModels").onclick=()=>refreshModelSelect(pid,{...(old||{}),key:$("#key")&&$("#key").value.trim()||old&&old.key||"",baseUrl:$("#base")&&$("#base").value.trim()||old&&old.baseUrl||"",accountId:$("#account")&&$("#account").value.trim()||old&&old.accountId||""});if(["gemini","openai","pollinations","aihorde","cloudflare"].includes(pid)&&!cachedModels(pid).length)refreshModelSelect(pid,{...(old||{}),key:old&&old.key||"",baseUrl:old&&old.baseUrl||"",accountId:old&&old.accountId||""});$("#save").onclick=()=>saveProvider(pid,old,p)}
