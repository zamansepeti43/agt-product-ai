...
if(pid==="pollinations")url="https://image.pollinations.ai/models";
...
const fallback=[...new Set([...(cachedModels(pid)),...(MODEL_CATALOG[pid]||[]),old&&old.model].filter(Boolean))];
sel.innerHTML=fallback.map(x=>"<option value=\""+esc(x)+"\">"+esc(x)+"</option>").join("");
if(old&&old.model)sel.value=old.model;
if(status)status.textContent="Model listesi yükleniyor…";
try{const list=await fetchProviderModels(pid,old);sel.innerHTML=list.map(x=>"<option value=\""+esc(x)+"\">"+esc(x)+"</option>").join("");if(old&&list.includes(old.model))sel.value=old.model;else if(list[0])sel.value=list[0];if(status)status.textContent=list.length+" model bulundu";}catch(e){if(status)status.textContent="Hazır liste kullanılıyor";}}
const PROVIDERS=[["pollinations","Pollinations","⚡",true,true,"qwen-image-3"],["aihorde","AI Horde","🌐",true,false,"AlbedoBase XL (SDXL)"],["gemini","Google Gemini","✨",false,true,"gemini-3.1-flash-image"],["openai","OpenAI","◉",false,true,"gpt-image-1"],["cloudflare","Cloudflare AI","☁️",false,true,"@cf/runwayml/stable-diffusion-v1-5-img2img"],["custom","Özel OpenAI","🔌",false,true,""],["generic","OpenAI uyumlu / Özel API","🔗",false,true,""],["comfyui","Yerel ComfyUI","🖥️",true,false,"Qwen Image Edit"]];
let state={file:null,recipe:"hero",slots:read("provider-slots")||[],selected:null,mode:read("provider-mode")||"free",count:1,ratio:"1:1",busy:false,results:[],extra:read("extra")||"",lang:read("agt-lang")||"tr",notify:read("agt-notify")!==false};
...
function write(k,v){try{if(window.AGTNativeVault){window.AGTNativeVault.set(k,JSON.stringify(v));return}}catch(e){}try{localStorage.setItem(k,JSON.stringify(v))}catch(e){}}
...
function openProviderModal(){$("#modal").classList.remove("hidden");$("#sheetTitle").textContent=L().addTitle;$("#sheetBody").innerHTML='<div class="provider-grid" id="providerGrid"></div><div id="form"></div>';$("#close").onclick=()=>$("#modal").classList.add("hidden");PROVIDERS.forEach(p=>{const b=document.createElement("button");b.innerHTML="<strong>"+p[2]+"</strong><span>"+p[1]+"</span><small>"+(p[3]?L().free:"API key")+"</small>";b.onclick=()=>showForm(p[0]);$("#providerGrid").appendChild(b)})}
function showForm(pid){const p=provider(pid),old=state.slots.find(x=>x.provider===pid),t=L();$("#providerGrid").style.display="none";let models=[...new Set([...(cachedModels(pid)),...(MODEL_CATALOG[pid]||[]),old&&old.model].filter(Boolean))];let h='<div class="form"><h3>'+p[2]+" "+p[1]+'</h3><label>'+t.name+'<input id="label" value="'+esc(old&&old.label||p[1])+'"></label>';if(p[4])h+='<label>'+t.key+'<input id="key" type="password" placeholder="••••••••"></label>';if(pid==="cloudflare")h+='<label>'+t.account+'<input id="account" value="'+esc(old&&old.accountId||"")+'"></label>';if(["custom","generic","comfyui"].includes(pid))h+='<label>'+t.base+'<input id="base" value="'+esc(old&&old.baseUrl||(pid==="comfyui"?"http://127.0.0.1:8188":""))+'"></label>';if(pid==="comfyui")h+='<label>Workflow<textarea id="workflow">'+esc(old&&old.workflow||"")+'</textarea></label>';h+='<label>'+t.model+'<div class="model-picker"><select id="model">'+models.map(x=>"<option value=\""+esc(x)+"\">"+esc(x)+"</option>").join("")+'</select><button type="button" id="refreshModels">↻</button></div><small id="modelStatus">'+(models.length?models.length+" model hazır":"Model seç")+'</small></label><button class="save-btn" id="save">'+t.save+'</button><small class="secure-note">'+t.secure+'</small></div>';$("#form").innerHTML=h;if(old&&old.model&&models.includes(old.model))$("#model").value=old.model;$("#refreshModels").onclick=()=>refreshModelSelect(pid,{...(old||{}),key:$("#key")&&$("#key").value.trim()||old&&old.key||"",baseUrl:$("#base")&&$("#base").value.trim()||old&&old.baseUrl||"",accountId:$("#account")&&$("#account").value.trim()||old&&old.accountId||""});if(["gemini","openai","pollinations","aihorde","cloudflare"].includes(pid)&&!cachedModels(pid).length)refreshModelSelect(pid,{...(old||{}),key:old&&old.key||"",baseUrl:old&&old.baseUrl||"",accountId:old&&old.accountId||""});$("#save").onclick=()=>saveProvider(pid,old,p)}

function saveProvider(pid,old,p){const key=$("#key")&&$("#key").value.trim()||old&&old.key||"";if(p[4]&&!key){toast(L().key,"error");return}const n={id:old&&old.id||crypto.randomUUID(),provider:pid,label:$("#label").value.trim()||p[1],key:key,model:$("#model").value.trim()||p[5],baseUrl:$("#base")&&$("#base").value.trim()||old&&old.baseUrl||"",accountId:$("#account")&&$("#account").value.trim()||old&&old.accountId||"",workflow:$("#workflow")&&$("#workflow").value||old&&old.workflow||""};state.slots=old?state.slots.map(x=>x.id===n.id?n:x):state.slots.concat(n);write("provider-slots",state.slots);state.selected=n.id;state.mode="manual";write("provider-mode","manual");$("#modal").classList.add("hidden");render();toast("✓ "+L().ai,"success")}
async function generate(){if(state.busy)return;if(!state.file){toast(L().needFile,"error");return}let list;if(state.mode==="free")list=[...state.slots].sort((a,b)=>Number(provider(b.provider)&&provider(b.provider)[3])-Number(provider(a.provider)&&provider(a.provider)[3]));else if(state.mode==="paid")list=[...state.slots].sort((a,b)=>Number(provider(a.provider)&&provider(a.provider)[3])-Number(provider(b.provider)&&provider(b.provider)[3]));else list=[state.slots.find(x=>x.id===state.selected)].filter(Boolean);if(!list.length){toast(L().needAI,"error");return}state.busy=true;render();let last="";for(const s of list){try{state.results=await callProvider(s);state.busy=false;render();toast("✓ "+L().done,"success");nativeNotify(L().done,L().notifyText);return}catch(e){last=e.message||String(e)}}state.busy=false;render();toast(L().error+last,"error")}
async function callProvider(s){
  ...
  const runOne=async(index)=>{
    ...
    if(s.provider==='pollinations'){
      ...
      const r=await fetch(u);if(!r.ok)throw Error('Pollinations API hatası');return[{url:URL.createObjectURL(await r.blob())}];
    }
...
function download(x,i){const a=document.createElement('a');a.href=x.url;a.download='AGT-'+String(i+1).padStart(2,'0')+'.png';document.body.appendChild(a);a.click();a.remove()}
render();