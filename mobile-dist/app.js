...
let state={file:null,recipe:"hero",slots:read("provider-slots")||[],selected:null,mode:read("provider-mode")||"free",count:1,ratio:"1:1",busy:false,results:[],history:Array.isArray(read("agt-history"))?read("agt-history"):[],favorites:Array.isArray(read("agt-favorites"))?read("agt-favorites"):[],nav:"home",extra:read("extra")||"",lang:read("agt-lang")||"tr",notify:read("agt-notify")!==false};
...
function renderResults(){const box=$("#results");if(!box)return;box.replaceChildren();state.results.forEach((x,i)=>{const f=document.createElement("figure"),img=document.createElement("img"),actions=document.createElement("div"),b=document.createElement("button"),fav=document.createElement("button");f.className="result-item";img.loading="lazy";img.decoding="async";img.src=x.url;actions.className="result-actions";b.textContent=L().download;b.onclick=()=>download(x,i);fav.className="favorite-btn "+(isFavorite(x)?"on":"");fav.textContent=isFavorite(x)?"♥":"♡";fav.title=isFavorite(x)?"Favoriden çıkar":"Favorilere ekle";fav.onclick=()=>toggleFavorite(x);actions.append(b,fav);f.append(img,actions);box.appendChild(f)})}
function renderNav(){
